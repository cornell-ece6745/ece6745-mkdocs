...
On the ECELinux, source the course setup scripts and clone the repo. Be
sure to open your remote desktop GUI as well.
...
Normally, circuit designers will write a reference **Spice schematic** of
the intended circuit first and then simulate it to ensure the high-level
functionality is correct before moving onto layout. Spice is a language
used to define how various hardware elements are connected, along with
any parameters such as RC parasitics for devices. In this schematic, we
will not include extracted RC parasitics, we will instead extract these
values from our layout and re-simulate using the same testbench later in
the lab.
